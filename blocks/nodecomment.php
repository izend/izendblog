...
function nodecomment($lang, $node_id, $node_url, $nomore, $moderate) {
	...
	switch($action) {
		...
		case 'delete':
			...
			if (!($moderate or ($comment_user_id == $user_id and $comment_created + 15*60 > $now))) {
				$id=false;
				$bad_id=true;
				break;
			}
	...
	$newcomment=$user_name=$user_page=false;
