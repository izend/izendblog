...
function configure($lang) {
	...
	switch($action) {
		case 'init':
			$sitename='myblog.net';
			$webmaster='webmaster@myblog.net';
	...
	switch($action) {
		case 'configure':
			...
			try {
				init_db($db_host, $db_name, $db_user, $db_password, $db_prefix, $site_admin_user, $site_admin_password, $site_admin_mail, $default_language, $sitename);
			}
			catch (PDOException $e) {
				$db_error=$e->getMessage();
				break;
	...
	$output = view('configure', $lang, compact('token', 'sitename', 'webmaster', 'db_error', 'file_error', 'internal_error', 'content_languages', 'default_language', 'db_reuse', 'db_admin_user', 'db_admin_password', 'db_type', 'db_name', 'db_host', 'db_prefix', 'db_user', 'db_password', 'site_admin_user', 'site_admin_password', 'errors'));
